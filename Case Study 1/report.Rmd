...
O histograma mostra uma distribuição inclinada à direita, que indica que a média é maior do que a moda e sugere a presença de outliers. Em relação ao Q-Q, percebe-se que os dados não seguem um padrão linear, que denota que a amostra  não provém de distribuição padrão normal. Finalmente, o gráfico de caixa reforça a presença dos dados em torno da média levemente inferior a 50.

Devido à presumível falta de normalidade dos dados foi feito o teste de Shapiro-Wilk para confirmação. A Hipótese Nula desse teste assume que amostra veio de uma população com distribuição normal e para isso o parâmetro  W é calculado  e comparado com um valor de referência. Caso a resultante de W seja inferior ao tabelado, rejeita-se a hipótese de normalidade, considerando o nível $\alpha$ de significância.  A aplicação do teste gerou como resultado $W = 0.94772$ e o valor de $p = 0.00755$, que confirmam a premissa de não normalidade dos dados.

Apesar disso, como o número de observações é suficientemente grande, o Teorema do Limite Central pôde ser evocado para estimação da média.
