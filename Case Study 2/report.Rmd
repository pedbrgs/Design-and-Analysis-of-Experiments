...
Diferentemente do cálculo realizado quanto às amostras masculinas, no qual o método de d de Cohen pôde ser utilizado, ele não é adequado para estimação do tamanho de efeito das amostras femininas, pois $F2017$ não apresenta características de normalidade, como mostrado pelo teste de Shapiro Wilk. Devido a isso, outras abordagens presentes da bibliografia foram examinadas, dentre elas uma medida baseada em probabilidade, conhecida como $A$ ou estatística do tamanho do efeito da linguagem comum ($CL$), que não é sensível à não normalidade dos dados ou diferença de variâncias, como o d de Cohen \cite{Ruscio2008}.

$A$ está intimamente relacionado a várias outras estatísticas que requerem apenas dados ordinais para estimar a diferença entre dois grupos, incluindo as estatísticas de teste não paramétrico Wilcoxon Rank Sum e Mann-Whitney U, bem como a área sob uma curva de característica de operação do receptor ($ROC$) calculada usando o método trapezoidal Hanley & McNeil de 1982 \cite{Ruscio2008}.
...
```
Com a medida de $A$ baseada em probabilidade, os valores nas populações 1 e 2 são considerados do mesmo tamanho, diferenças igual a zero, se $A = 0,5$ \cite{Delaney2002}. Considerando então que $A =  0,8571429$, podemos identificar diferenças maiores que $4$ entre as amostras femininas, $F2016$ e $F2017$.
